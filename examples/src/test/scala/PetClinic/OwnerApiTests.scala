...
class OwnerApiTests extends AnyFunSuite with BeforeAndAfter:
  ...
  test("'toMg' should a simple request") {
    ...
    val expected = Set((0, "request to OwnerApi.addOwner. Expecting code 201", 1), (1, "request to OwnerApi.getOwner. Expecting code 200", 2), (2, "request to OwnerApi.deleteOwner", 3), (3, "", 0))
    assert(result == expected)
  }
    test("'toMg' should handle choice (branching)") {
      val result = toMg(choose(addThenGet, listOwners))
      val expected =  Set((0, "request to OwnerApi.listOwners. Expecting code 200", 3), (0, "request to OwnerApi.addOwner. Expecting code 201", 1), (1, "request to OwnerApi.getOwner. Expecting code 200", 2))
      assert(result == expected)
    }

    test("'toMg' should handle choice succeeded by sequence ") {
      val result = toMg(choose(addThenGet, addDeleteYield) >> {_ => listOwners})
      val expected =   Set((4, "", 5), (2, "", 5), (0, "request to OwnerApi.addOwner. Expecting code 201", 1), (0, "request to OwnerApi.addOwner. Expecting code 201", 3), (1, "request to OwnerApi.deleteOwner. Expecting code 204", 2), (3, "request to OwnerApi.getOwner. Expecting code 200", 4), (5, "request to OwnerApi.listOwners. Expecting code 200", 6))
      assert(result == expected)
    }
